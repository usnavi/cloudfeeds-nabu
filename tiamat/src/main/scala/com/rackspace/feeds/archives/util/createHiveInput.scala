...
object CreateHiveInput {
  ...
  def main( args: Array[String] ): Unit = {
    ...
    // val users = Array( User( "708786", "MossoCloudFS_45a38ef8-838a-4dac-8562-a2e8ca525df2", "reach" ))

    val feeds = List( "backup/events" )
    val nasty_feeds = List( "files/events" )
//    val total_entries = 9130000
    val total_entries = 90
    ...
    nasty_feeds.zipWithIndex.foreach { case( feed, fI ) =>
...
e.eventtype, e.tenantid, e.region, e.date, e.feed;
